        ...
        static void Main(string[] args)
        {
            ...
            while (input.Equals("End") == false)
            {
                ...
                if (tokens.Length == 1)
                {
                    ...
                    foreach (string patient in departmentsData[currDepartment])
                    {
                ...
                else if (tokens.Length == 2)
                {
                    ...
                    if (int.TryParse(tokens[1], out roomNum))
                    {
                        ...
                        foreach (string patient in departmentsData[tokens[0]]
                            .Skip(skip)
                            .Take(3)
                            .OrderBy(p => p))
                        {
                    ...
                    {
                        foreach (string patient in doctorsData[input]
                            .OrderBy(p => p))
                        {
                            Console.WriteLine(patient);
                        }

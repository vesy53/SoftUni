            ...
            for (int i = 0; i < commandsCount; i++)
            {
                ...
                switch (command)
                {
                    ...
                    case "Add":
                        ...
                        Console.WriteLine(
                            clinics.FirstOrDefault(c => c.Name.Equals(args[2])).AddPet(currentPet));
                        break;

                    case "Release":
                        // Release {clinic's name}
                        Console.WriteLine(
                            clinics.FirstOrDefault(c => c.Name.Equals(args[1])).Release());
                        break;

                    case "HasEmptyRooms":
                        // HasEmptyRooms {clinic’s name}
                        Console.WriteLine(
                            clinics.FirstOrDefault(c => c.Name.Equals(args[1])).HasEmptyRooms());
                        break;
